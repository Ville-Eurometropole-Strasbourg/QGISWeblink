...
from QGISWeblink.utils.plugin_icons import PluginIcons
from qgis.core import Qgis, QgsLayerDefinition,QgsLayerTreeLayer,QgsProject,QgsMessageLog
TMPDIR = tempfile.TemporaryDirectory()

def download_ressource_file(url,localname):
        """recupere le fichier qlr"""
        retour = requests.get(url,verify=False)
        if retour.status_code == 200:
            # tmpdir = tempfile.TemporaryDirectory()
            tdn=TMPDIR.name
            fichier=os.path.join(tdn,localname)
            fichier=fichier.replace('\\','/')
            with open(fichier, "wb") as fich:
                fich.write(retour.content)
            layerfile=fichier
            return layerfile
        short_message=u"ressource introuvable: {0}".format(url)
...
class WmsLayersTreeNode(FavoritesTreeNode):
    ...
    def get_qgis_layer_details(self):
        ...
        qgis_layer_uri_details = [{
            ...
            "provider": "wms",
            "title": name,
            "uri": u"crs={}&featureCount=10&format={}&layers={}&styles={}&url={}".format(
...
class WmsStyleLayerTreeNode(FavoritesTreeNode):
    ...
    def __init__(self, title, node_type=PluginGlobals.instance().NODE_TYPE_WMS_LAYER_STYLE,
                 description=None, status=None, metadata_url=None, ident=None, params=None, parent_node=None):
        """
        """

        FavoritesTreeNode.__init__(self, title, node_type, description, status, metadata_url, ident, params, parent_node)
...
class QlrLayerTreeNode(FavoritesTreeNode):
    """
    Tree node for a qlr definition
    """
    ...
    def run_add_to_map_action(self):
        ...
        #     qgis_layer_details["title"],
        #     qgis_layer_details["provider"])
        layerfile = qgis_layer_details["uri"]
        if layerfile:
